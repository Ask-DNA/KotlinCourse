...
private fun String.justify(width:Int): String {
    ...
    if (difference > (width / 2)) // won't justify strings, which length is too short
        return this
    ...
    while (difference > 0) {
        if (ret[index].isWhitespace() && !sameWhitespace){
...
private fun MutableList<String>.wordWrap(width: Int) {
    ...
    do {
        ...
        if(splitPoint > 0) {
            ...
            this[index]=this[index].substring(0, splitPoint)
